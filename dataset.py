...
class BaseAnomalyDetectionDataset(Dataset):
    def __init__(self, split, class_name, img_size, dataset_path):
        ...
        self.rgb_transform = transforms.Compose([
            # 调整图像大小到 224x224 像素，使用 BICUBIC 插值方法
            transforms.Resize((RGB_SIZE, RGB_SIZE), interpolation = transforms.InterpolationMode.BICUBIC),
            # 将图像从 PIL 格式转换为 PyTorch 的张量格式，且像素值归一化到 [0, 1] 范围
            transforms.ToTensor(),
            # 使用之前定义的均值和标准差对图像进行标准化处理，使每个通道的数据分布符合标准正态分布
...
class TrainValDataset(BaseAnomalyDetectionDataset):
    ...
    def __getitem__(self, idx):
        ...
        # 返回一个元组，包含处理后的 RGB 图像、调整后的 3D 点云数据、调整后的深度图，以及样本的标签。这个元组可以直接用于训练或验证模型
...
class TestDataset(BaseAnomalyDetectionDataset):
    ...
    def __getitem__(self, idx):
        ...
        # 返回一个元组，包含处理后的 RGB 图像、调整后的 3D 点云、调整后的深度图，地面真值图像，样本标签以及 RGB 图像的路径
...
def get_data_loader(split, class_name, dataset_path, img_size = 224, batch_size = 1, shuffle = False):
    ...
    data_loader = DataLoader(dataset = dataset, batch_size = batch_size, shuffle = shuffle, num_workers = 1, drop_last = False, pin_memory = True)
